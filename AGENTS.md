...
This repo was originally built for Claude Code, which auto-loads
`.claude/skills/skillweb/SKILL.md` as a "skill." If you are a different coding
agent (Codex CLI, opencode, Cursor, Aider, etc.) and were just pointed at this
repo's git URL, follow the instructions below manually — there is no special
tooling required, just read the files.
...
1. **Read `.claude/skills/skillweb/SKILL.md` in full before writing anything.**
...
2. **Read every reference file `SKILL.md` links under
   `.claude/skills/skillweb/references/`** that applies to the current
...
- `.claude/skills/skillweb/SKILL.md` — the full specification (Korean). Authoritative.
- `.claude/skills/skillweb/references/*.md` — validated, reusable code snippets, in the priority order `SKILL.md` §4 lists.
- `.claude/skills/skillweb/Skilltech.md` — a build log / rationale doc, useful for context but not itself instructions.
...
Unzip `skillweb-skill.zip` into your own project's `.claude/skills/` directory
and start a new Claude Code session — skills are scanned once at session
start, so it won't appear mid-session. After that it auto-triggers when you
hand it a project `.md` and ask for a "발표자료"/web deck, or invoke it
directly.
...
- Or copy `.claude/skills/skillweb/SKILL.md` and its `references/` folder
